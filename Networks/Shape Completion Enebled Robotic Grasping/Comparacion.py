...
y = np.load(datasets_directory + 'labels_40.npy')
x = np.load(datasets_directory + 'output 40.npy')

elementos = y.shape[0]
resultados = []
for elemento in range(elementos):
	print(elemento)
	salida_red = x[elemento,:,:,:].reshape((patch_size,patch_size,patch_size))
	etiqueta = y[elemento,:,:,:,:].reshape((patch_size,patch_size,patch_size))
	views = getviews_from_voxel_cube(salida_red,patch_size)
	views2 = getviews_from_voxel_cube(etiqueta,patch_size)
...
np.save('/media/leo/Datasets/resultados.npy',resultados)
