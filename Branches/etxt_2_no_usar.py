...
def scalesize(x_np_pts, scale = 255,labels = False, center = 0, factor_scala = 0 ):
    #--------------------------------------set home
    ...
    if labels == True:
        ...
        for i,array in enumerate(cloud):
            cloud2.append(array + center[i])

        cloud2 = check_error(cloud2,scale, place = 'etiquetas')  
        return cloud2
    ...
    for array in cloud:
    	...
    	if maximo < scale:
            ...
            cloud2.append(array)
            center.append(0)
    return cloud2, center, factor_scala


def check_error(cloud2, scale, place = 'Desconocido'):
    x_array, y_array, z_array = cloud2
    x_max = np.max(x_array)
    y_max = np.max(y_array)
    z_max = np.max(z_array)
    maximo = np.max([x_max,y_max,z_max])
    cloud = []
    if maximo>scale-1:
        tem = []
        for array in cloud2:
            ajuste = (maximo-(scale-1))
            tem.append(ajuste)
            cloud.append(array-ajuste)
        print('Forzando Datos en ', place,'por: ', tem)
    else:
        cloud = cloud2
    return cloud        
...
def getviews(x_np_pts,scale,offset = False,labels = False, center = 0, factor_scala = 0):
    if offset == False:
        offset = int(scale*.1 + 1) 
    scale = scale
    image = np.zeros((scale, scale,6))
    if labels==False:
        cloud, center, factor_scala = scalesize(x_np_pts, scale, labels = False, center = 0, factor_scala = 0)
    else:
        cloud = scalesize(x_np_pts, scale, labels = True, center = center, factor_scala = factor_scala)
